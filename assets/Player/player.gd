...
func _physics_process(delta: float) -> void:
	...
	if is_on_floor() and horizontal_velocity.length() > 0.1 and not footsteps.is_playing():
		footsteps.play()
	
	# Se não estivermos no chão OU estivermos parados, paramos o som
	elif (not is_on_floor() or horizontal_velocity.length() < 0.1) and footsteps.is_playing():
		footsteps.stop()
	move_and_slide()
...
func _walk(delta: float) -> Vector3:
	move_dir = Input.get_vector(&"movement_left", &"movement_right", &"movement_forward", &"movement_backward")
